(() => {
  ...
  document.addEventListener('keydown',event=>{if(phase==='closed'||phase==='custom'||(race&&host.inert)||event.code!=='Escape')return;event.preventDefault();if(drag){cancelDrag();highlight(null);selected=null;update();}else if(phase==='levels')close();else showLevels();});
  function finish(){
    if(race){if(phase==='playing'&&C.evaluate(L,items).ready)race.onComplete(JSON.parse(JSON.stringify(items)));return;}
  ...
  window.SalvageGame={open:showLevels,start:startLevel,close,startRace(level,callbacks){if(race){cleanScene();levels.pop();}race=callbacks;levels.push(level);startLevel(levels.length-1);},stopRace(){if(!race)return;race=null;cleanScene();clearTimers();cancelDrag();levels.pop();phase='closed';host.inert=false;host.hidden=true;host.innerHTML='';}};
})();
